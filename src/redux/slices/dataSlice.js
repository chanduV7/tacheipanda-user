...
const dataSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(filterJobs.fulfilled, (state,action) =>{
      state.value.filteredJobs= action.payload;
    });
    builder.addCase(filterJobs.rejected, (state,action) =>{
      state.error= action.error;
    });
...
export const filterJobs = createAsyncThunk("filterJobs", async({filterInp}) => {
  const token = localStorage.getItem("token");

  const {data} = await axios.get(baseUrl3 +`/search/search?search=${filterInp}`,
  {
    headers : {
      Authorization : "Bearer " + token
    }
